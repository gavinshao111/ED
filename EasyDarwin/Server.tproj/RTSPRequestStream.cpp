            ...
            if (0 != rc)
                ...
                if (!videoReqInfo.ignore) {
                    ...
                    if (0 == rc2) {
                        ...
                        StrPtrLen fullFileNameStr(fullFileName, lenOfFullFileName);
                        if (!IsUrlExistingInSessionMap(fullFileNameStr)) {
                            //if(true){
...
#if 1                                        
                            int rc3 = waitForPush(fullFileName, lenOfFullFileName, timeToWaitForPush);
                            if (0 == rc3)
                                usleep(1000 * 100);
                            else {
                                if (-2 == rc3) {
                                    fprintf(stderr, "[INFO] %.*s: waitForPush time out (%ds).\n\n", lenOfFullFileName, fullFileName, timeToWaitForPush);
                                } else if (-1 == rc3)
