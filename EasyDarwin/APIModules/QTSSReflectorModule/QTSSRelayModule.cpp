...
QTSS_Error SetupAnnouncedSessions(QTSS_StandardRTSP_Params* inParams)
{
	...
	UInt32 theLen = 0;
	QTSS_Error theErr = QTSS_GetValuePtr(inParams->inRTSPRequest, qtssRTSPReqMethod, 0, (void**)&theMethod, &theLen);
...
QTSS_Error RouteAuthorization(QTSS_StandardRTSP_Params* inParams)
{
	...
	// Get the Remote IP address
	UInt32* theIP = 0;
	QTSS_Error theErr = QTSS_GetValuePtr(inParams->inRTSPSession, qtssRTSPSesRemoteAddr, 0, (void**)&theIP, &theLen);
	if ((theErr != QTSS_NoErr) || (theLen != sizeof(UInt32)))
	{
		Assert(0);
		return QTSS_NoErr;
